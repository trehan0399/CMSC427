...
public:
    ...
    Sphere(const Vec3f &c, const float &r, const float kd, const float ks)
        : center(c), radius(r), radius2(r * r), Kd(kd), Ks(ks) {}
...
public:
    ...
    Vec3f color1 = Vec3f(0.1, 0.5, 0.2); 
    Vec3f color2 = Vec3f(0.8, 0.2, 0.1);  
    float scale = 10;
    float pattern = (std::sin(st.x * scale) + std::sin(st.y * scale)) * 0.5; 
    ...
    const std::vector<std::unique_ptr<Light>> &lights)
{
    ...
    for (uint32_t j = 0; j < options.height; ++j) {
        for (uint32_t i = 0; i < options.width; ++i) {
            Vec3f colorSum(0.0f, 0.0f, 0.0f);        
            for (int s = 0; s < N; ++s) {
                float x = (i + static_cast<float>(rand()) / RAND_MAX) / options.width;
                float y = (j + static_cast<float>(rand()) / RAND_MAX) / options.height;
                float sx = (2 * x - 1) * imageAspectRatio * scale;
                float sy = (1 - 2 * y) * scale;
                Vec3f dir = normalize(Vec3f(sx, sy, -1));
                colorSum += castRay(orig, dir, objects, lights, options, 0);
            }     
            Vec3f finalColor = (colorSum / static_cast<float>(N));
            *(pix++) = finalColor;
        }
    }
    ...
    std::ofstream ofs;
    ofs.open("./example.ppm", std::ofstream::binary);
    if (ofs.fail()) {
        std::cout << "failed to open out.png!" << std::endl;
    }
...
int main(int argc, char **argv)
{
    ...
    Sphere *sph1 = new Sphere(Vec3f(-1, 0, -12), 2, 0.8, 0.2); 
    sph1->materialType = DIFFUSE_AND_GLOSSY;
    sph1->diffuseColor = Vec3f(0.2, 0.4, 0.8);
    Sphere *sph2 = new Sphere(Vec3f(0.5, -0.5, -8), 1.5);
    sph2->ior = 1.3;
    sph2->materialType = REFLECTION_AND_REFRACTION;
    sph1->center = Vec3f(-2, 1, -10);
    sph1->radius = 3;
    sph1->radius2 = sph1->radius * sph1->radius;
